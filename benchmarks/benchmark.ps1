...
$Script:PythonExe = Join-Path $ProjectRoot "benchmarks\runtime\python\python.exe"
$Script:PyTestsDir = Join-Path $BenchmarkDir "tests\py"
$Script:LuaExe = Join-Path $ProjectRoot "benchmarks\runtime\lua\lua54.exe"
...
function Invoke-Benchmarks {
    ...
    if (Test-Path $PythonExe) {
        ...
        Write-Host "  - $pyVersion (comparison enabled)" -ForegroundColor Green
        $hasRuntime = $true
    }

    if (Test-Path $LuaExe) {
        $luaVersion = & $LuaExe -v 2>&1
        Write-Host "  - $luaVersion (comparison enabled)" -ForegroundColor Green
        $hasRuntime = $true
    }

    if (-not $hasRuntime) {
        Write-Host "  - no runtime available" -ForegroundColor Yellow
    }
    Write-Host ""
