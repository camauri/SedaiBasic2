...
| `type<T>(args)` / `T(args)` / `= (a,b,c)` | ✓ | Anonymous UDT temporary with an explicit type; aggregate/tuple initialisation of a constructor-less UDT, including array-of-UDT `{(a,b), (c,d)}`. As in fbc, a type with a constructor — declared, or implied by a field initialiser, a variable-length `String` field, a dynamic array member, a member or base that has one, or `Extends Object` — is refused a list of values (a destructor alone does not count); `Type(args)` then calls a matching constructor. |
...
pointer subtraction are unaffected in both cases — `*p`, `p[i]`, `p2 - p1` and
`For p = @a(0) To @a(n)` all answer what FreeBASIC answers, because both sides of those speak the same
unit.
...
| `ZSTRING` | ✓ | Null-terminated string type (`DIM AS ZSTRING [* n]`); `ZSTRING PTR` is a raw pointer to a string's bytes (see `SADD`). An array of `ZString * n` is n-byte cells, as in fbc, so `@a(i)` is the address of cell i. ⚠️ **An array parameter `z() As ZString * n` names the argument's elements here, and in fbc it does not**: fbc reads it with a one-byte stride (`z(1)` is the second character of `a(0)`) and a write through it does not reach the argument. That is a defect of theirs, reported in `job/fbc-upstream/ISSUES.md` n. 9, and this product does not copy it (owner, 24 Sep 2026) - a declared divergence, DIVERGENZE 617. |
...
| `PROCPTR (Procedure pointer and vtable index)` | ✓ | Address of a procedure (= @p). A MEMBER is named through its type — `ProcPtr( T.proc )`, `ProcPtr( T.constructor, Sub( ByVal As Integer ) )`, `ProcPtr( T.let )`, `ProcPtr( T.+= )`, `ProcPtr( T.[] )`, `ProcPtr( T.cast, Function() As Integer )`, and a property's getter/setter told apart by `Function`/`Sub` — and the optional signature picks the overload (its parameter count *and* its types); with no signature the first declared one wins. The result is a procedure whose FIRST parameter is the object, `Sub cdecl( ByRef As T, … )`, so it is called as `p( obj, … )`. ⚠️ The vtable-index form is refused by name. A call THROUGH the pointer works as a direct call: an argument left out takes the default the pointer's TYPE declares, and a `String` for a `ZString Ptr` parameter passes its address (DIVERGENZE 534, 535). ⭐ A procedure written into a record field that C reads is the machine address C can jump to, and read back it is the procedure again (DIVERGENZE 561). 🕳️ One declared limit: an address that reaches a procedure-pointer VARIABLE through RAW BYTES - `memcpy` of a vtable - is callable from C and from BASIC, but compared with `@proc` it answers 0 where fbc answers -1, because that road passes through no typed field read. A procedure pointer whose function returns a RECORD by value is called like the function itself - `c = v(1)`, `c = h.cf(1)`, and the field of what it returns, `v(1).r` (DIVERGENZE 592) - and one whose signature takes or returns a STRUCT BY VALUE crosses to C in both directions: a C function pointer read from a library's memory is called, and a procedure stored in a record C calls - a colour of four Singles in, sixteen bytes back in two registers - is a real callback (DIVERGENZE 565). A callback may take a `va_list` parameter (it is C's, and handed on to `vsnprintf` it is C's again, DIVERGENZE 582), and a `cdecl` function may RETURN a procedure to C, which C then calls; to a BASIC caller the result is the procedure again (DIVERGENZE 583). A `ZString * n` whose address the program takes goes to a C pointer parameter as its own buffer, so C may keep it or write into it (DIVERGENZE 578). A pointer FIELD of a record in the variadic tail of a C routine (`printf("%s", s.name)`) goes as the pointer it is (DIVERGENZE 579). `x.f[i]` on a `ZString * n` field reads and writes byte i of the field (DIVERGENZE 571). An unsized `ByRef w As WString` / `ZString` parameter whose address is taken keeps its text, and `@w` points at its characters; a write through `@w` does not reach `w` (DIVERGENZE 569). A C routine may `longjmp` out of a callback (a fatal CUnit assertion): the program goes on and ends normally (DIVERGENZE 577). `pp[i]->field` on a `T Ptr Ptr` over native memory (`Callocate`, C's) reads and writes the record at `pp + i` (DIVERGENZE 586). So does an indexed pointer EXPRESSION - `(*pp)[i].f`, `h.items[i].f` - whose records live in native memory (DIVERGENZE 595). A pointer to a `ZString * n` (or `StrPtr(s)`) held in a cell C follows through a `ZString Ptr Ptr` (`strsep(@p, ",")`) is the machine address for the call, and what C moves comes back (DIVERGENZE 587). |
...
| `-> (Pointer to member access)` | ✓ | `p->field` member access through a UDT pointer/handle (equivalent to `p.field`). The pointer may be what a CALL returns, with no variable in between - a BASIC function declared `As T Ptr` or as an alias of it (`Type pT As T Ptr`), or a C function: the field keeps its type (an unsigned one prints with no sign column) and a pointer field can be dereferenced, `*f()->name` (DIVERGENZE 558 · 575 · 594). The address of a record handed to C and handed back by C - callback user data, `pthread_getspecific` - is that record again, `String` fields included (DIVERGENZE 566). |
...
| `Placement New` | ◐ | `NEW(address) T`, `NEW(address) T(args)` and `NEW(address) T[n]` construct AT the address, as in FreeBASIC, for a builtin type and for a record whose values are C bytes (the plain types and the types with methods, constructors or a destructor, in the default memory mode): the memory is cleared, the fields get their defaults, the constructor runs, the answer is the address itself, and the vector form writes no element count in front. ⚠️ For the other records - a type with a base type, a `String` field, or the `strict` memory mode - the address expression is evaluated and an ordinary instance is constructed elsewhere, so the program is right in everything except the object's ADDRESS. |
...
| Keyword | Status | Description |
|---|---|---|
| `Declare` | ✓ | Forward `DECLARE SUB|FUNCTION ...` is accepted and ignored (calls are resolved by a pre-pass over the definitions). ⭐ **A `DECLARE` that carries an `ALIAS` and has no body is a FOREIGN procedure** (8 Sep 2026): its signature is recorded and a call to that name is compiled into a call to the C function. Supported parameter and return types: the integer widths (`Byte`…`ULongInt`, `Boolean`), `Single`, `Double`, any `... Ptr`, and `String`/`ZString`/`WString` (the address of the bytes). ⛔ A type that has no C meaning here is **refused by name**, not guessed at — a wrongly marshalled argument does not raise, it returns wrong numbers. Two are refused today: a **UDT passed BY VALUE** (the calling convention implements it and its net exercises it; the language surface does not route it yet) and **`LongDouble`**, which is an 80-bit x87 value travelling on the x87 register stack on SysV and a plain `Double` on Win64 — one declaration, two meanings. ⚠️ The calling convention is SedaiBasic's own on both targets, with no external dependency; it covers SysV AMD64 and Win64, and x86-64 only. A call to a C routine may omit trailing arguments its declaration gives a DEFAULT (`byval b as long = 10`, `= -5`, `= @"abc"`, `= SOME_CONST`): the defaults are applied, as fbc does (DIVERGENZE 557). |
...
| Keyword | Status | Description |
|---|---|---|
| `PRINT` | ✓ | ⚠️ **The same output on every system (owner, 25 Sep 2026).** fbc on Windows prints a floating-point number with a THREE-digit exponent (`1e-006`, `6.02214199e+023`, the Microsoft C runtime's way) and on Linux with two (`1e-06`); this product prints two everywhere, the Windows build included. And a `WSTRING` printed to a redirected file goes out as UTF-8 text everywhere, where fbc on Windows writes raw UTF-16 units. fbc's per-platform answers belong to the PLATFORM mode (see `WSTRING`). Declared divergences, DIVERGENZE 619 and 620; measured by the Windows layout net. |
| `? (Shortcut for 'PRINT')` | ✓ | `?` is a shorthand for `PRINT`. |
| `PRINT USING` | ✓ | Sign positions: `+` first or last prints `+`/`-`; `-` **first or last** prints `-` for a negative and a **blank** for a positive. A leading sign owns its own field position, so it never causes the `%` overflow marker by itself; a sign with no position asked for competes with the digits (`"#.###"` on `-1.5` gives `%-1.500`). **Was wrong until 12 Aug 2026**: a `-` in first position was silently dropped, so every negative overflowed its field. ⭐ **The digits are fbc's (23 Sep 2026, DIVERGENZE 584)**: in MODERN a Double is first turned into a 16-digit decimal, rounded, and those digits are then rounded to the mask - as FreeBASIC's runtime does - so `2.675` with `#.##` prints `2.68` although its binary value lies just below the tie. A number with more than three integer digits beyond the field's positions is printed in floating point behind a `%` (`"#.####"` on 983057.1 gives `%0.9831E+6`), and an exponent wider than the carets carries its own `%` (`0.1E+%301`). CLASSIC keeps the Commodore formatter unchanged. |
...
| Keyword | Status | Description |
|---|---|---|
| `INPUT #` | ✓ | Reads a list of values from a file or device. A numeric field is read with the SAME grammar `VAL` uses — the `&H`/`&O`/`&B` base prefixes, a saturating magnitude, the full 64 bits — and a `BOOLEAN` destination takes the words `true`/`false` in either case, anything else through that grammar with non-zero meaning true. (Until 26 Aug 2026 it had its own conversion, 32-bit and prefix-blind: `Val("&h1F")` was 31 and `INPUT #` of the same text was 0. Guard `m584`.) A string field follows FreeBASIC's rule (7 September 2026, guard `m881`): leading blanks and tabs are not part of it, trailing blanks are, a field opening with a quote runs to the closing quote with its commas and without the quotes, and what follows the closing quote is the next field. |
...
| `PUT #` | ✓ | Writes arbitrary data to a file or device. `Put #f, pos, a(i), n` (and `Get #f, pos, a(i), n`) moves `n` ELEMENTS starting at the one named, as fbc does (DIVERGENZE 559). |
...
| `EXEPATH` | ⚠️ | `EXEPATH` (bare or `EXEPATH()`) → **the directory of the BASIC PROGRAM being run**, and `COMMAND(0)` → its full path. FreeBASIC compiles the program INTO the executable, so there the two questions have one answer; here the executable is the interpreter, and answering *its* directory made the universal FB idiom `ExePath + "/data/..."` work under `fbc` from every directory and under `sb` from none. A **declared divergence**, deliberate: the answer is the program's own home, stable from any working directory. A relative path that does not exist in the current directory is also looked up **beside the program** — for `DIR`, `FILEEXISTS` and a **read-only** open only; a write always creates exactly where the program said. `--home` runs the program as if from its own directory (opt-in; the default keeps the current directory the shell's, as `fbc` does). Declared 9 September 2026. ⭐ **A FUSED executable answers exactly as fbc's** (`sbc --fuse prog.bas` → `prog`, the runtime with the program inside, 23 September 2026): the program IS the executable, so `EXEPATH` is its directory, `COMMAND(0)` its path, every command-line argument is the program's, a relative path is looked up in the current directory only, and a C library that looks next to "the executable" (`/proc/self/exe`, `GetModuleFileName`) finds the program's directory — which under `sb prog.bas` it cannot (DIVERGENZE 573, declared, as for Python and Lua). |
...
| `UTFTOWCHAR` | ⚠️ | `UTFToWChar(encod, src, dst, chars)` — the wide twin. ⭐ **A UTF-16 surrogate pair decodes WHOLE here, and in fbc it does not**: fbc computes the codepoint in a sixteen-bit intermediate, so U+1F4A9 comes back as `&hF4A9`, U+10FFFF as `&hFFFF` and U+10000 as **zero** — which the caller reads as the terminator, losing the rest of the string. That is a defect of theirs, reported in `job/fbc-upstream/ISSUES.md` n. 4, and this product does not copy it (owner, 17 Sep 2026: *we copy everything, but not the defects*) — a declared divergence, DIVERGENZE 514. A high surrogate with no low one after it is not a pair: the unit stands for itself. Needs `#include "utf_conv.bi"`. |
...
| `SCREEN (Graphics) and SCREENRES` | ✓ | `SCREENRES w,h[,depth[,num_pages]]` sets the graphics screen surface; `SCREEN n` selects a numbered QB/FB mode (1/7→320×200, 13→320×200, 18→640×480, 19→800×600, 20→1024×768, 21→1280×1024, …) mapped to a resolution. Both allocate pages and route through IGraphicsBackend (headless-testable via SCREENINFO). ⭐ **The DEPTH is honoured** (8 Sep 2026) and the default when it is absent is fbc's **8**: 1, 2, 4 and 8 give an indexed surface of one byte per pixel through a 256-colour palette, 15 and 16 give 16bpp (two bytes, RGB565), 24 and 32 give 32bpp — the same rounding fbc does, checked against it on all eight. `SCREENINFO` answers what you got, and `SCREENPTR` and `IMAGEINFO`'s pixel pointer walk the surface at THAT width. ⚠️ `SCREEN n` sets the resolution and leaves the depth at the default; the per-mode depth fbc gives each numbered mode is a declared divergence. `SCREENRES` also has the FUNCTION form fbc accepts, `r = ScreenRes(w, h, ...)`, which answers 0. ⭐ The 5th argument — the driver flags of `fbgfx.bi` — is honoured since 7 September 2026: `GFX_FULLSCREEN` (1) asks `sb --window` for the whole monitor (desktop mode, letterboxed), `GFX_WINDOWED` (0) for a window, and a later `SCREENRES` switches; other flags are accepted and ignored. |
...
**An address kept in an `Integer` comes back as a pointer**, as in FreeBASIC: `Cast(Integer, p)`, `Str(p)` and `Print p`
give the machine address, and `Cast(T Ptr, n)` of that number (or of that number plus an offset) reads, writes and
compares like `p`. The one exception is internal: a number that names an element of one of the arrays that still live
in the VM (strings, `Boolean`, pointers - see above) is taken as that element again, which is what `@a(i)` gave.
