...
type
  ...
  private
    ...
    FPendingChar: Char;                    // Buffered character from KeyPressed
    FHasPendingChar: Boolean;              // True if FPendingChar is valid
...
var
  ...
  KeySym: Integer;
begin
  // If we already have a pending character, return True
  ...
  begin
    case Event.type_ of
      ...
      SDL_KEYDOWN:
      begin
        // Handle special keys that don't produce SDL_TEXTINPUT
        ...
        case KeySym of
          ...
          SDLK_SPACE:
          begin
            // Space might not generate TEXTINPUT in some cases
            FPendingChar := ' ';
            FHasPendingChar := True;
            Result := True;
            Exit;
          ...
          // For regular letters (a-z), check if printable ASCII
          // These will come via SDL_TEXTINPUT, so don't block here
          // Just continue the loop to catch SDL_TEXTINPUT
        end;
      end;
    end;
  end;
...
begin
  // Check if we have a pending character (set by ProcessEvents or KeyPressed)
  Result := FHasPendingChar;
end;

function TSDL2InputDevice.GetLastChar: string;
begin
  // First check for a pending character from KeyPressed
  if FHasPendingChar then
  begin
    Result := FPendingChar;
    FHasPendingChar := False;
  ...
  begin
    // No pending char, call ReadKey for blocking wait
    Result := ReadKey;
    if Result = #0 then
      Result := '';
  end;
...
procedure TSDL2InputDevice.EnableTextInput;
begin
  SDL_StartTextInput;
end;
    ...
    begin
      case Event.type_ of
        ...
        begin
          ...
          // Uses standard ASCII/Unicode control codes
          ...
          begin
            ...
            case KeySym of
              ...
              SDLK_RETURN, SDLK_KP_ENTER:
              begin
                FPendingChar := #13;  // CR (Carriage Return)
                FHasPendingChar := True;
              end;
              SDLK_ESCAPE:
              begin
                FPendingChar := #27;  // ESC (Escape)
                FHasPendingChar := True;
              end;
              SDLK_BACKSPACE:
              begin
                FPendingChar := #8;   // BS (Backspace)
                FHasPendingChar := True;
              end;
              SDLK_DELETE:
              begin
                FPendingChar := #127; // DEL (Delete)
                FHasPendingChar := True;
              end;
              SDLK_TAB:
              begin
                FPendingChar := #9;   // HT (Horizontal Tab)
                FHasPendingChar := True;
              end;
              SDLK_SPACE:
              begin
              ...
              // Cursor keys - use Unicode Private Use Area (U+E000-U+E0FF)
              // These will be returned as single UTF-8 characters
              SDLK_UP:
              begin
              ...
              // Function keys F1-F12 use codes 128-139
              SDLK_F1:
              begin
                FPendingChar := #128; // F1
                FHasPendingChar := True;
              ...
              SDLK_F10:
              begin
                FPendingChar := #137; // F10
                FHasPendingChar := True;
              end;
              SDLK_F11:
              begin
                FPendingChar := #138; // F11
                FHasPendingChar := True;
              end;
              SDLK_F12:
              begin
                FPendingChar := #139; // F12
                FHasPendingChar := True;
              end;
            end;
          end;
