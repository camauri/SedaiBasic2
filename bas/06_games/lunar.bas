5 REM http://www.vintage-basic.net/bcg/lunar.bas
...
150 PRINT L,INT(A);INT(5280*(A-INT(A))),INT(3600*V*100+.5)/100,INT(M-N):INPUT K:T=10 
...
240 PRINT "FUEL OUT AT";INT(L*100+.5)/100;"SECONDS":S=(-V+SQR(V*V+2*A*G))/G
250 V=V+G*S: L=L+S
260 W=3600*V: PRINT "ON MOON AT";INT(L*100+.5)/100;"SECONDS - IMPACT VELOCITY";INT(W*100+.5)/100;"MPH" 
...
310 PRINT "IN FACT, YOU BLASTED A NEW LUNAR CRATER";INT(W*.227*100+.5)/100;"FEET DEEP!"
320 GOTO 440
